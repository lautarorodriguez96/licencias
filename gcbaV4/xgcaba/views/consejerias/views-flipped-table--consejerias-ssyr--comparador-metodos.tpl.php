  ...
  <?php if ($header_row) : ?>
    <thead>
      <tr>
        <th class="first">
			<div>
				<p class="text-compara">Compar&aacute; m&eacute;todos</p>
				<p>y eleg&iacute; cu&aacute;l es el m&aacute;s adecuado para vos</p>
			</div>
        </th>
  ...
  <tbody>
    <?php foreach ($rows_flipped as $field_name => $row) : ?>
      <tr class="<?php print $row_classes_flipped[$field_name]; ?>">
        <th class="<?php print "fila-" . ++$contadorfila; ?>">
          <div class="comparador-th"><?php echo $header[$field_name]; ?></div>
        </th>
        <?php foreach ($row as $index => $item): ?>
          <td class="<?php print (++$columnas % 3 == 0 ? 'columna' : '') . " fila-" . $contadorfila; ?>">
            <?php echo $item; ?>
          </td>
        <?php endforeach; ?>
      </tr>
